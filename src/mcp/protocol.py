...
class JSONRPCResponse(BaseModel):
    ...
    def check_consistency(cls, values):
        ...
        if (
            "error" in values
            and values["error"] is not None
            and "result" in values
        ...
        if "id" in values and values["id"] is not None:
            if ("error" not in values or values["error"] is None) and (
                "result" not in values
            ):
                raise ValueError(
                    "JSON-RPC response must include result or error when id is present."
                )
        return values
