...
def _build_content(
    summary: str, metadata: dict, snapshot: str, warnings: list[str], provenance: dict
) -> str:
    lines = [
        ...
        "",
        "=== DISCLAIMER ===",
        "This report was generated automatically and should be reviewed by a",
        "qualified toxicologist before use in regulatory or safety decisions.",
        "Predictions based on QSAR models are estimates with applicability limits.",
        "",
    ]

    if warnings:
        lines.append("=== APPLICABILITY DOMAIN WARNINGS ===")
        for warning in warnings:
            for chunk in wrap(warning, MAX_CHARS) or [""]:
                lines.append(chunk)
        lines.append("")
    ...
    if provenance:
        ...
        for key, value in provenance.items():
            for chunk in wrap(f"{key}: {value}", MAX_CHARS) or [""]:
                lines.append(chunk)
        lines.append("")
...
def generate_pdf_report(log_data: dict) -> BytesIO:
    ...
    provenance = {
        "Source": "OECD QSAR Toolbox WebAPI",
        "QSAR models run": str(len(qsar_results)),
        "AD warnings": str(len(warnings) - 1) if warnings else "0",
    }
    ...
    content = _build_content(summary, metadata, serialized, warnings, provenance)
