...
def test_create_checkpoint_if_missing_does_not_duplicate(orchestrator):
    cp1 = orchestrator.create_checkpoint_if_missing("wf-1", "chemical_identity", {"chem_id": "123"})
    cp2 = orchestrator.create_checkpoint_if_missing("wf-1", "chemical_identity", {"chem_id": "456"})
...
def test_submit_review_approves(orchestrator):
    ...
    updated = orchestrator.submit_review(cp.checkpoint_id, "user-1", ReviewDecision.APPROVED, "Looks good")
...
def test_submit_review_rejects(orchestrator):
    ...
    updated = orchestrator.submit_review(cp.checkpoint_id, "user-1", ReviewDecision.REJECTED)
...
def test_expired_checkpoint_auto_rejected(orchestrator):
    cp = orchestrator.create_checkpoint("wf-1", "chemical_identity", {}, expires_minutes=-1)
